import os, time, datetime, pytz, random, string
...
while True:
  ...
  if menu == "A" or menu == "1":
    ...
    addGame = string.capwords(addGame)
...
Tags        : {games[sel]["Tags"]}"""
    ...
    elif which == "T" or which == "7":
      x = input("One tag, or multiple tags ")
      if x == "one":
        choices = []
        tag = input("Which tag? ")
        for game in games:
          if tag in games[game]["Tags"]:
            choices.append(game)
        sel = random.choice(choices)
        print(form())
        update = input("Update last played to now? ").strip().upper()
        ...
        y = input("1.Any game that has at least one of the tags will be selected from\n2.Only games that have ALL of the tags will be selected from")
        if y == "a" or y =="1":
          t = ""
          TAGS = []
          while t != "DONE":
            t = input("Enter a tag or DONE to finish ")
            if t != "DONE":
              TAGS.append(t)
          choices = []
          for game in games:
            for x in games[game]["Tags"]:
              if x in TAGS and game not in choices:  
                choices.append(game)
          print(choices)
          sel = random.choice(choices)
          print(form())
          ...
          for game in games:
            choices.append(game)
            for tag in TAGS:
              if tag not in games[game]["Tags"]:
                choices.remove(game)
                
          #need to control for error when no games fit criteria    
          sel = random.choice(choices)
          print(form())
          update = input("Update last played to now? ").strip().upper()
          if update[0] == "Y":
